...
**Teljesítve valódi hardveren 2026-09-12.** Az `install` a belső SATA-lemezre tette a rendszert, a netbook a belső lemezről indul, boot → prompt **208 ms** (a Phase 1-es 192 ms + AHCI-init és lemez-csatolás). A perzisztens írás, a `lastpanic` és a `poweroff` a felhasználó jelentése szerint működik. A `tests/smoke.py` négy boot-menetben 60 ellenőrzést futtat: ring 3-as taskok, 22 syscall, capability-elutasítás audit-naplóval, CPU-/határidő-limit (a `spin` programot a kernel öli meg), ramfs, AOFS v2 formázás, perzisztens írás újraindításon át, panic-tároló szektor, `install` és a telepített rendszerről bootolás. Mért értékek: kernel.bin 94 KB, ramdisk 126 KB (a telepítőhöz a bootloader és a kernel is benne van), boot → prompt 113 ms.
...
**Teljesítve valódi hardveren 2026-09-13.** A netbookon (RTL8101E, MAC 08:9e:01:5d:a8:cb, DHCP-vel 192.168.1.237, átjáró 192.168.1.1, boot 209 ms) az `agent coder irj egy hello.txt fájlt a /project/src alá` parancs a PC-n futó hídon (192.168.1.70:9010, `claude-sonnet-5`, közepes erőfeszítés) át egyetlen `fs_write` eszköz-hívással létrehozta a `/project/src/hello.txt` fájlt ("Hello, AO-OS!"), a válasz magyar ékezetekkel jelent meg, a futás 5,5 s volt. Az első HW-körben talált problémák és javításaik: a modell UTF-8-at küld, ezért a konzol, a billentyűzet és a sorszerkesztő UTF-8-ra állt át; hiányzó könyvtárnál az agentnek nem volt eszköze és a hibaüzenet érthetetlen volt, ezért jött az `fs_mkdir` eszköz, a szülőkönyvtárak automatikus létrehozása és a magyarázó hibaüzenetek; a híd rendszerüzenete kimondja, hogy nincs shell és nincs külső program; hívásszám-korlát a végtelen próbálkozás ellen. A ping-válaszidő 19,8 ms-ról az eseményvezérelt várakozással a hálózati valós értékre csökken. A PSK-titkosítás (3.1) lentebb.

**Szolgáltató-függetlenség igazolva (2026-09-13):** a híd `--provider` kapcsolóval Claude (Anthropic SDK, `claude-sonnet-5`) vagy Gemini (Google AI Studio REST, `gemini-2.5-flash`, külön csomag nélkül) mögé áll; a netbookon az `ai szia` mindkettővel válaszolt, a PC-s AOP-kliens tesztben a Gemini is helyes `fs_write` + `done` eszközhívást adott, és a netbookon az `agent coder` feladatot 2,1 s alatt végezte (Claude Sonnet 5 közepes erőfeszítéssel: 5,5 s; az eltérés főleg a gondolkodási beállításból jön, `--effort low` a Claude-ot is gyorsítja). Az OS-en egyetlen bájt sem változott a váltáshoz, pontosan ahogy a 11. fejezet tervezte.

**Phase 3.1 (PSK-titkosítás) QEMU-ban igazolva (2026-09-13):** AOP 1.1: a kézfogásban mindkét fél 8 bájtos nonce-t ad, az alkulcs HChaCha20(PSK, nonce-ok), utána minden keret ChaCha20-Poly1305 (nonce = irány ‖ számláló, AAD = a fejléc, ismétlés elutasítva). Implementáció `user/crypto.c` (RFC 8439 tesztvektorok: `run cryptotest`) és `tools/aocrypto.py`; a keretezés és a kézfogás a `tools/aop.py`-ban közös a híd, a szimulált híd és a PC-s kliens között. A kulcs a PC-n `~/.ao-psk` (`bridge.py --gen-psk`), a netbookon `/state/ai/psk`; kulcs nélküli klienst a híd elutasít, titkosítatlan hidat a kulcsos kliens nem fogad el. A smoke-teszt 4. menete titkosított csatornán futtatja a coder-agentet, és az `update` parancsot is igazolja (a `/state` megmarad). **HW-n igazolva 2026-09-13:** a netbook `ai szia` parancsa `[hid: model=gemini-2.5-flash, titkositott csatorna]` jelzéssel, 1,18 s alatt válaszolt; a kulcs a `/state/ai/psk`-ban, a híd `~/.ao-psk`-val futott. Ezzel a Phase 3 minden pontja teljesült.

A tesztelés közben talált két hiba és javításuk: (1) az AOX-képek 0-tól voltak linkelve abban a hitben, hogy a kód RIP-relatív, de a fordító adat-táblái (pl. a `switch` mutató-táblája az `explain()`-ben) abszolút címeket tartalmaznak, így az agentd az első hibaüzenetnél (`E_CAP`, `E_NOENT`) rossz címről olvasott; a képek most a rögzített 0x400000-ra linkelődnek (`user/aox.ld`), ahogy a 6. fejezet eredetileg is mondta. (2) Ring 3-as kivételnél a kernel az egész gépet megállította; most csak a taskot állítja le `E_FAULT` (-17) kóddal, a shell fut tovább (`run fault` teszt). A netbookra telepített képben az (1) még benne van: egy elutasított agent-írás piros `KIVETEL 14: page fault` képernyőt ad, amíg az új kép fel nem kerül.
...
**4.0 elkészült, QEMU-ban igazolva (2026-09-13, 104 lépéses smoke-teszt):** `/state/rc` (vagy `/etc/rc`) indító szkript, `append` parancs a szerkesztéséhez; Tab-kiegészítés parancsnevekre és útvonalakra (közös előtag, több találat listázva); `time`/`date` és `set` alparancsuk a CMOS RTC-ből (`kernel/drv/rtc.c`, BCD/12 órás módok kezelve); `shot` (a konzol szövege UTF-8-ban a híd `shots/` mappájába), `copy [N]` (az előző parancs kimenete vagy N sor a PC vágólapjára), `paste [FÁJL]` (a PC vágólapja a parancssorba előtöltve vagy fájlba), mind az `agentd` `--file`/`--clip` módján és a `clip` manifesten át, a titkosított csatornán (AOP 12–14 típusok); `help` témák szerint két oszlopban, `help PARANCS`; kezdőképernyő állapotsorral (`status` parancs is), és a hálózati task a DHCP után AOP PING-gel ellenőrzi a hidat. Közben javítva: ring 3-as kivétel csak a taskot állítja le; a TCP a TIME_WAIT-es helyet újrahasználja, ha elfogytak a foglalatok; az agent-indítás megvárja a boot-kori DHCP/PING-et. A verzió 0.2. **HW-n igazolva 2026-09-13:** az első `shot` a netbookról a PC-re érkezett (kezdőképernyő, `[hid: … elerheto, PSK van]`, titkosított Gemini-válasz 1,26 s, Tab-lista), boot 224 ms.
...
**4.1 elkészült, QEMU-ban igazolva (2026-09-13):** a lenyomat-séma a `docs/AOP.md`-ben (AOP 15 `PROJECT`, 16 `IMPRINT`); a híd oldalán `tools/projector.py` (csak stdlib: letöltés gzip-pel és karakterkódolással, HTMLParser-alapú kinyerés, zajszűrés tag- és osztálynév-minták alapján, link-sűrűség heurisztika, súlyozás, 60 KiB-os méretkorlát a legkisebb súlyú csomópontok elhagyásával); a netbookon `user/projector.c` (~600 sor) a `user/json.c` részhalmaz-elemzővel és az `user/aop.c` közös AOP-klienssel (az agentd is erre épül már). Három oszlop (vázlat, tartalom, linkek), szín = típus, halvány = kis súly, inverz = fókusz; navigáció nyilakkal, Tab, Enter (link megnyitása új lenyomatként, előzményekkel), Backspace, PgUp/PgDn, `/` keresés, `s` mentés a `/state/projector/` alá, `q`. `--dump` szöveges kimenet tesztekhez és agenteknek. Kérdésnél már most a DuckDuckGo HTML-találati oldalát projektálja (a 4.2 első fele). Kernel: `ESC[sor;oszlopH` kurzorpozicionálás, halasztott sortörés az utolsó cellánál (VT100-módra; enélkül a teljes képernyős rajz görgetett), `sysinfo` adja a konzol méretét. Valódi próbák a PC-n: hu.wikipedia.org cikk és egy keresés helyes lenyomatot ad. HW-n kipróbálandó.
...
4. **FPU/SSE** (kész): `fxsave`/`fxrstor` a kontextusváltásban, CR0/CR4 beállítás, tiszta sablon az új taskoknak; a programok SSE-vel fordulnak, a `crt0` 16-ra igazítja a vermet. `run fputest`: két task lebegőpontos sorozata taskváltásokkal is bitre azonos a referenciával.
5. **Felhasználói `malloc`** (kész): `SYS_SBRK` a bss után növeli a heapet a manifest `mem` korlátján és egy ésszerű plafonon belül; `user/malloc.c` first-fit szabadlista összevonással, szabványos nevekkel (`malloc`, `free`, `calloc`, `realloc`). `run malloctest` (2 MiB blokk, 200 vegyes blokk, realloc), a 4 MiB-os manifest alatt a 6 MiB kérés NULL, nem összeomlás.
6. **Doom fut (QEMU-ban igazolva, 2026-09-13):** `user/doom.c` a PureDOOM egyfájlos motort (id Software / Daivuk, GPL, `user/doom/PureDOOM.h`, 48 600 sor) köti az AO-OS-hez tizenkét visszahívással: fájlok a `/state/games` alól (`SYS_SEEK`-kel), memória a saját `malloc`-ból, idő a tickből, kép az `fb` capability-n át (320×200 RGBA → 960×600 a képernyő közepén, 35 Hz), billentyűk a nyers konzolmódból (nyilak, Ctrl tűz, Space használat, Shift futás, Alt oldalazás, Esc menü). A PureDOOM változtatás nélkül fordult le a freestanding toolchainnel. Az 554 KB-os `doom.aox` nem fér a 1 MiB-os boot-területre a ramdiskkel együtt, ezért a `share/` mappába kerül és `fetch doom.aox /state/games`-szel települ; ez egyben az első "telepített alkalmazás" a rendszeren. A `mkimage` most hibát ad, ha a boot-terület megtelne. Az első képernyőkép: az E1M1 demó fut (`build/doom1.png`). **HW-n igazolva 2026-09-13:** a netbookon a `doom1.wad` 8,4 s alatt ment át, a játék elindult és játszható; a képernyőkép (`docs/img/doom-netbook.png`) az OS saját F12-es funkciójával készült és a hídon át jött a PC-re. Közben a TCP küldő oldala csúszó ablakot kapott (a stop-and-wait a Windows késleltetett ACK-ja miatt 7 KB/s volt), és a képernyőkép-szál (`kernel/drv/screenshot.c`) került a kernelbe.

### Phase 6: "streambook" (2026-09-13-tól)

A Doom-út mellékhatása egy irány: a netbook egyre inkább a képernyő és a billentyűzet, a PC a számítás, közte a saját, titkosított csatorna. Ezt tudatosan vállaljuk. Az első lépés a **projector hű nézete**: a PC-n egy valódi böngészőmotor (Chromium, Playwright-tal, egy dedikált munkaszálban) rendereli az oldalt, a kép zlib-bel tömörítve jön (AOP 18 `RENDER`, 19 `RENDERED` + FILE-darabok), a netbook a saját `inflate`-tel bontja ki (RFC 1951, puff-algoritmus, `user/inflate.c`) és az `fb`-re rajzolja (`user/gfx.c`: téglalap, RGB-blit, szöveg a rendszer betűkészletével). A híd a linkek helyét is küldi, a netbook keretet rajzol a fókuszált linkre: Tab lép, Enter megnyit, Backspace vissza, a görgetés helyi (három képernyőnyi kép jön egyszerre, a szélén újabb darab). A `v` billentyű vált a lenyomat és a hű kép közt ugyanarról az oldalról; `SYS_FB_RELEASE` adja vissza a konzolt a programnak kilépés nélkül. Egy Wikipédia-oldal: 9,4 MB nyers kép 329 KB tömörítve, 107 link. QEMU-ban a szimulált híd szintetikus képével és ellenőrző összeggel igazolva; a PC-n a `python tools\render.py <url>` külön próbálható. HW-n a linkkeretek először máshová estek (a teljes oldalas képernyőkép más elrendezést adott, mint a látható nézet), ezért a kép és a linklista ugyanabból a nézetből készül, csak a látható linkekkel.

**Feltöltési sebesség (2026-09-13):** a netbookról a 3 MB-os képernyőkép 160 KB/s-mal ment, a `net` számlálói szerint 26 újraküldéssel, 42 nulla-ablakkal és 401 ablakkorláttal. A szűk keresztmetszet nem a netbook volt, hanem a híd: a ChaCha20-Poly1305 tiszta Pythonban 0,24 MB/s-mal bontott, a PC fogadópuffere megtelt, a TCP nulla ablakot hirdetett, és az 500 ms-os újraküldési időzítő diktálta az ütemet. A híd most a `cryptography` csomag C-implementációját használja, ha van (940 MB/s ugyanazokkal az RFC 8439-es tesztvektorokkal; nélküle tartalékként marad a tiszta Python, és a híd indításkor figyelmeztet). A méréshez `netbench CÍM PORT [MB]` (nyers TCP a `tests/sink.py` felé) és a `net` TCP-számlálói kerültek a shellbe.

**Hálózati frissítés (2026-09-13):** a fejlesztői kör eddig `ao.py usb` (rendszergazda, diskpart), pendrive, boot, `update` volt. Most: a PC-n `ao.py build`, a netbookon `update`. A build a lemezkép első 2047 szektorát (MBR, stage2, kernel, ramdisk) `share/boot.img`-ként is kiírja, és egy build-bélyeget (dátum + git hash, `rootfs/etc/build`, illetve a stage2-terület utolsó 64 bájtja) tesz a képbe. A netbook `update` parancsa a meglévő FETCH-csel lehúzza a képet, ellenőrzi (0x55AA, `AOS2`, a fejléc LBA-i az 1 MiB-en belül, AOFS v1 szuperblokk a ramdisknél), a bélyeget összeveti a futóval, majd a kernel+ramdisk területet 64 KiB-os darabokban írja és visszaolvassa, utána a stage2-t, legvégül az MBR-t, amelyben a lemez saját partíciós táblája marad. Ezután újraindul. Az első induláskor a shell szála (`disk_sync_from_ramdisk`; a 16 KiB-os boot-verem kevés a rekurzív másoláshoz) észreveszi, hogy a ramdisk bélyege eltér a lemez `/etc/build`-jétől, és átmásolja a `bin/`-t és `etc/`-t; ezt csak akkor teszi, ha a lemez boot-területe ugyanezt a ramdiskt tartalmazza (`disk_booted_from_disk`: a stage2-fejléc és a ramdisk első/utolsó szektora egyezik), így egy régi pendrive-ról indulva nem írja felül a telepített rendszert. A pendrive-os út (`update rd`, `install`) megmaradt első telepítéshez és mentéshez. A/B boot-terület nincs: áramszünet írás közben esetén a pendrive a mentőút. A smoke-ban a szimulált híd a valódi `share/boot.img`-t adja, a QEMU a saját lemezképét frissíti, és az 5. menet ellenőrzi az első boot másolását. **HW-n igazolva 2026-09-13:** a netbookon a boot.img 90 ms alatt jött le, az írás és az újraindulás után a splash az új bélyeget és a "frissitve" sort mutatta, a `/state` (rc, billentyűzetkiosztás) megmaradt; három egymás utáni buildet vitt át pendrive nélkül.
...
**7.2 MicroPython (kész, QEMU-ban igazolva 2026-09-15):** a MicroPython v1.26.0 magja (`py/`, MIT), a `shared/` REPL- és readline-kódja, a musl-alapú `lib/libm_dbl`, a `re1.5` és néhány extmod-modul (os, time, random, re, json, heapq, binascii, vfs) a `user/micropython/` alá került, változtatás nélkül. A port (`user/micropython/port/`) négy fájl: `mpconfigport.h` (az "extra" szint dupla float-tal és mpz nagy egészekkel; aszinkron, select, tömörítés, hálózat, hardver-modulok ki), `mphalport.c` (konzol be/ki, idő, alvás a rendszerhívásainkkal; a szöveges konzolmód Enter-e `\r`-ré fordítva a readline-nak), `vfs_ao.c` (a MicroPython VFS-protokollja az `ao_open/read/write/seek/list/stat/mkdir/unlink` hívásokon, `open()`, `os.*`, import fájlból; a relatív utat a kernel oldja fel a task munkakönyvtárához képest, a jogokat a kernel ellenőrzi, az elutasítás `OSError(EACCES)`), `modao.c` (az `ao` modul: sysinfo, screen, ticks, sleep, getkey, con_mode, caps). A libc-t egy tucat fejléc és a `libc_ao.c` pótolja (a `memcpy`/`strlen` a kernel string-könyvtárából, a `malloc` a sajátunkból, a `printf` a MicroPython `shared/libc/printf.c`-jéből); a `-nostdlibinc` miatt a fordító beépített `stdint.h`-ja megmarad. Az NLR (kivételek) natív x86-64 assembly, a GC a regisztereket és a vermet is bejárja (`gchelper_generic.c`), a verem 1 MiB (a `crt0` `AOX_STACK` paramétere), a heap 16 MiB a `malloc`-ból. A build GNU make nélkül megy: a `tools/mpbuild.py` a MicroPython saját `py/*.py` szkriptjeivel generálja a qstr-, modul- és root-pointer-fejléceket (a `mkrules.mk` lépései: elődolgozás `-DNO_QSTR`-rel, split/cat, a `Q(...)` sorok idézőjelben az elődolgozón át), aztán clanggal fordít (129 core-fájl + libm + port), és a saját crt0-val linkel. Eredmény: `share/python.aox`, 316 KB; a netbookra `fetch python.aox /state/bin`, a shell `python` parancsa a `python.cap` manifesttel indítja. Kikapcsolt: a natív `_Float16` (nincs compiler-rt), a `copysign` és `log2` a portból jön. A smoke 4. menete: `-c`, szkript 2**100-zal, modul importja `/state/lib`-ből, REPL (`print`, `os.listdir`, `open` írás/olvasás), Ctrl+D.

**Vágólap a PC és a netbook közt, Ctrl+V bárhol (kész, 2026-09-15):** a kernel kapott egy vágólap-szálat (`kernel/drv/clipboard.c`, a képernyőkép-szál mintájára). A Ctrl+V-t a billentyűzet-lekérdezés fogja el szöveges módban, a szál a clip-agentet futtatja a `clip.cap` jogaival (`agentd --clip /tmp/clip.txt`, ugyanaz, mint a shell `paste`-je), majd a kapott szöveget billentyűleütésként adja be a futó programnak (`kbd_inject`), `KEY_PASTE_ON`/`KEY_PASTE_OFF` jelek közt, amiket a szöveges konzolmód a szabványos `ESC[200~` / `ESC[201~` (bracketed paste) sorozatként ad át. Így a beillesztés a shell parancssorában, a szerkesztőben és a Python REPL-ben is működik, program-oldali támogatás nélkül; a szerkesztő ismeri a jeleket, és köztük nyersen szúr be (nincs behúzás-öröklés, a tab marad, közben nem rajzol), a végén kiírja a sorok számát. A másik irány a programoké: a szerkesztő Ctrl+K-ja a kivágott sort a `/tmp/clip.txt`-n és az `agentd --file clip`-en át a PC vágólapjára is teszi (`SYS_SPAWN` a `clip.cap` szövegével). Nyers módban (játékok) a beillesztés eldobódik. A smoke: Ctrl+V a shellben lefuttatja a szimulált híd vágólapját (`echo vagolap-ok`), a szerkesztőben beilleszti, ^K a hídra küldi.

**7.3 F5 a szerkesztőből (kész, 2026-09-15):** `.py` fájlnál az F5 ment, letörli a képernyőt, és a `python.cap` jogaival elindítja a `/state/bin/python.aox`-ot a fájlra (`SYS_SPAWN`), a kimenet a konzolra megy, a végén "vége (rc=N)" és egy billentyű visszahoz a szerkesztőbe. A hibás sorra ugráshoz a MicroPython-port a `MICROPY_BOARD_AFTER_PYTHON_EXEC` horgon át minden kezeletlen kivétel tracebackjét a `/tmp/pyerr.txt`-be is kiírja; a szerkesztő ebből az utolsó, a saját fájljára mutató `File "...", line N` bejegyzést és az utolsó sort (a kivétel szövegét) veszi, a kurzort a sorra teszi, az üzenetet az állapotsorba írja. Közben a szöveges konzolmód az F1–F12 és az Insert billentyűket is xterm-sorozatként adja át (`ESC[15~` az F5), és a soros bemenet (QEMU-teszt) ezeket is érti.
